            ...
            SectionView(title: "IV. ВРЕМЕННЫЕ И СТРУКТУРНЫЕ ПОКАЗАТЕЛИ") {
                ...
                if let maxIncomeMonth = analytics.maxIncomeMonth {
                    MetricRow(
                        ...
                        note: "Показывает месяц с максимальным доходом за весь период.",
                        formula: "Максимальный доход = max(доходы по всем месяцам)\n\nОпределяется месяц с наибольшим значением дохода."
                    )
                }
                if let maxExpenseMonth = analytics.maxExpenseMonth {
                    MetricRow(
                        ...
                        note: "Показывает месяц с максимальными расходами за весь период.",
                        formula: "Максимальный расход = max(расходы по всем месяцам)\n\nОпределяется месяц с наибольшим значением расходов."
                    )
                }
                MetricRow(
                    label: "Доход / Расход",
